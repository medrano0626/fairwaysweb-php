...
const vtrno = document.getElementById('vtrno');
var checkstat = 0;
...
function submitForm() {
  ...
  let newvtrdate = vtrdate.slice(5,7) + '/' + vtrdate.slice(8,10) + '/' + vtrdate.slice(0,4)
  ...
  if (newvtrdate < periodfrom || newvtrdate > periodto){
...
vtrno.addEventListener('keydown', (e) => { // if enter key is press inside the vtrno inputbox
    if (e.key === 'Enter') {
        checkstat = 0;
        checkvtr();
        ...
        $.ajax({
           ...
           success: function callback(response){
            if(a == 0){
              ...
              document.getElementById("helpertripamount").focus();
            }
            if(a == 1){
              document.getElementById("drivertripamount").value = response 
              document.getElementById("drivertripamount").focus(); 
            }
